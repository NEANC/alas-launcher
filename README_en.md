...
- MacOS: Open the downloaded `.dmg` file, drag `AzurLaneAutoScript.app` into the `Applications` folder. Before first launch, run the command shown in the DMG background in Terminal: `xattr -dr com.apple.quarantine /Applications/AzurLaneAutoScript.app` (because I don't have an Apple developer certificate to sign the program)
